...
| Поле | Формат | Пример |
|------|--------|--------|
| ✏️ Название проекта | Текст (уникальное) | `MyApp` |
...
> ℹ️ **Примечание:** Локальный путь (`local_repo_path`) - служебное поле, управляется системой автоматически.

## Специальные значения

### Название проекта
- **Обязательное**: не может быть пустым
- **Уникальное**: должно быть уникальным в системе
...
Bot: ❌ Название проекта не может быть пустым!
...
```
[Нажать "✏️ Название проекта"]
ExistingProject  ← имя уже существует

Bot: ❌ Проект с названием ExistingProject уже существует! Выберите другое название.

UniqueProjectName  ← уникальное имя
Bot: ✅ Значение сохранено!
```

### Неверный формат групп
```
/edit_project MyApp
[Нажать "👥 Группы"]
